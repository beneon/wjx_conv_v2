...
from wjx_model.data_file_parser import DataFileIter, StudentInfo, AnswerEntry
from wjx_model.attachment_parser import AttachmentParser
from wjx_model.type_dict_coll import CourseConfigYaml
...
class Curriculum:
    ...
    def __init__(self,wd:str):
        ...
        self.student_data = DataFileIter(df_student_data, StudentInfo).iter_to_list()
    ...
    def __getitem__(self, item):
        if 0<=item<self.__len__():
            session_wd = os.path.join(self.wd,self.config_of_course['entries'][item])
            assert os.path.exists(session_wd), f'session wd:{session_wd} do not exist'
            return Session(session_wd,self.student_data)
        else:
...
class Session:
    def __init__(self,entry_path:str,student_data:DataFileIter[StudentInfo]):
        self.wd = entry_path
    ...
    def generate_session_data(self):
        return SessionData(self.student_data,self.answers)
    ...
    def _update_answer_fu_fields(self,answer_entry:AnswerEntry):
        ...
        for attachment in related_attachments:
            ...
            if len(related_answer_field_index_list)==1:
                ...
                raise Exception(f'found no answer field with q_title:{attachment.question_title}')
            else:
    ...
    def _load_answers(self):
        ...
        return DataFileIter(df_answer_data,AnswerEntry).iter_to_list(), session_id


class SessionData:
    def __init__(self, student_info:DataFileIter[StudentInfo], answer_entries:DataFileIter[AnswerEntry]):
        """
    ...
    def _update_ae_with_student_info(self,e:AnswerEntry):
        try:
            i = self.student_info.find_index_of_item_with_attr_val('student_name',getattr(e,'student_name'))
            if i:
